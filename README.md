...
```sh
curl -fsSL https://raw.githubusercontent.com/unlikefraction/silicon-browser/managed-v0.3.0/scripts/install.sh | sh && export PATH="$HOME/.local/bin:$PATH"
```
...
For installation without interactive setup, use `curl -fsSL https://raw.githubusercontent.com/unlikefraction/silicon-browser/managed-v0.3.0/scripts/install.sh | sh -s -- --no-setup`, then run `~/.local/bin/browser setup` when ready. You can also [download prebuilt binaries](https://github.com/unlikefraction/silicon-browser/releases/tag/managed-v0.3.0) or build this release using `cargo install --git https://github.com/unlikefraction/silicon-browser.git --tag managed-v0.3.0 --locked silicon-browser-cli` with Rust 1.98 or later. The crates.io release awaits publisher access; use the native packages or pinned Git source for 0.3.0. For a source checkout, use `cargo install --path crates/cli`.
