...
///
/// Resolution order:
/// 1. SILICON_BROWSER_SKILLS_DIR env var (points directly at a single directory)
...
fn find_package_root() -> Option<PathBuf> {
    if let Ok(exe) = env::current_exe() {
        ...
        if let Some(parent) = exe.parent() {
            // npm install layout: bin/silicon-browser-* -> ../
...
fn find_skills_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = env::var("SILICON_BROWSER_SKILLS_DIR") {
    ...
    if targets.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "No skill name provided. Usage: silicon-browser skills get <name>",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} No skill name provided. Usage: silicon-browser skills get <name>",
    ...
    if skills_dirs.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "Skills directory not found. Set SILICON_BROWSER_SKILLS_DIR or reinstall via npm.",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} Skills directory not found. Set SILICON_BROWSER_SKILLS_DIR or reinstall via npm.",
                color::error_indicator()
            );
        }
        exit(1);
