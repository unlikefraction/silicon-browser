...
async fn e2e_runtime_stream_enable_before_launch_attaches_and_disables() {
    ...
    guard.set("SILICON_BROWSER_SESSION", "e2e-runtime-stream");
...
async fn e2e_save_state_cross_domain() {
    ...
    let tmp_state = std::env::temp_dir()
        .join("silicon-browser-e2e-cross-domain-state.json")
        .to_string_lossy()
        .to_string();
    let resp = execute_command(
...
// silicon-browser (e.g. by the user or another CDP client) should be detected
...
async fn e2e_externally_opened_tab_detected() {
    ...
    // opens a tab while silicon-browser is connected via --cdp.
...
async fn e2e_relaunch_on_options_change() {
    ...
    let resp = execute_command(
        &json!({
            ...
            "action": "launch",
            "headless": true,
            "userAgent": "silicon-browser-test/1.0"
        }),
        &mut state,
    )
    .await;
...
async fn e2e_stream_frame_metadata_respects_custom_viewport() {
    let guard = EnvGuard::new(&["SILICON_BROWSER_SOCKET_DIR", "SILICON_BROWSER_SESSION"]);
    let socket_dir = std::env::temp_dir().join(format!(
        "silicon-browser-e2e-stream-viewport-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock should be after unix epoch")
    ...
    guard.set("SILICON_BROWSER_SESSION", "e2e-stream-viewport");
