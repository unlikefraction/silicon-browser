...
pub struct DaemonState {
    ...
    pub engine: String,
    /// Default timeout for wait operations, from SILICON_BROWSER_DEFAULT_TIMEOUT env var.
...
impl DaemonState {
    pub fn new() -> Self {
        Self {
            ...
            domain_filter: Arc::new(RwLock::new(
                env::var("SILICON_BROWSER_ALLOWED_DOMAINS")
                    .ok()
                    .filter(|s| !s.is_empty())
            ...
            pending_dialog: None,
            auto_dialog: !matches!(
                env::var("SILICON_BROWSER_NO_AUTO_DIALOG").as_deref(),
                Ok("1" | "true" | "yes")
            ),
            stream_client: None,
            stream_server: None,
            launch_hash: None,
            engine: env::var("SILICON_BROWSER_ENGINE").unwrap_or_else(|_| "chrome".to_string()),
            default_timeout_ms: env::var("SILICON_BROWSER_DEFAULT_TIMEOUT")
                .ok()
    ...
    /// configured `default_timeout_ms` (from `SILICON_BROWSER_DEFAULT_TIMEOUT`).
        ...
        loop {
            match rx.try_recv() {
                ...
                Err(broadcast::error::TryRecvError::Lagged(n)) => {
                    eprintln!("[silicon-browser] Warning: CDP event buffer overflowed, {} events dropped. Network requests may be missing from HAR output.", n);
                    continue;
                }
...
async fn auto_launch(state: &mut DaemonState) -> Result<(), String> {
    ...
    // Cloud provider: when SILICON_BROWSER_PROVIDER is set, connect via the
    ...
    if let Ok(provider) = env::var("SILICON_BROWSER_PROVIDER") {
...
fn launch_options_from_env() -> LaunchOptions {
    ...
    LaunchOptions {
        ...
        proxy_username: env::var("SILICON_BROWSER_PROXY_USERNAME").ok(),
        proxy_password: env::var("SILICON_BROWSER_PROXY_PASSWORD").ok(),
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let launch_options = LaunchOptions {
        ...
        proxy_username: cmd
            ...
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("SILICON_BROWSER_PROXY_USERNAME").ok()),
        proxy_password: cmd
            .get("proxy")
            .and_then(|v| v.get("password"))
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("SILICON_BROWSER_PROXY_PASSWORD").ok()),
        profile: cmd
            .get("profile")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
...
fn write_extensions_file(session_id: &str) {
    if let Ok(val) = env::var("SILICON_BROWSER_EXTENSIONS") {
...
fn get_har_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".silicon-browser").join("tmp").join("har")
    } else {
        std::env::temp_dir().join("silicon-browser").join("har")
    }
}
...
mod tests {
    ...
    async fn test_stream_disable_clears_state_when_stream_file_removal_fails() {
        ...
        guard.set("SILICON_BROWSER_SESSION", "stream-disable-cleanup-session");
    ...
    async fn test_stream_enable_port_conflict_returns_error() {
        ...
        guard.set("SILICON_BROWSER_SESSION", "stream-port-conflict-session");
    ...
    async fn test_daemon_state_new() {
        let guard = EnvGuard::new(&[
            "SILICON_BROWSER_ALLOWED_DOMAINS",
            "SILICON_BROWSER_SESSION_NAME",
            "SILICON_BROWSER_SESSION",
        ]);
        guard.remove("SILICON_BROWSER_ALLOWED_DOMAINS");
        guard.remove("SILICON_BROWSER_SESSION_NAME");
        guard.remove("SILICON_BROWSER_SESSION");
    ...
    async fn test_handle_har_stop_without_path_uses_default_location() {
        ...
        assert_eq!(har["log"]["creator"]["name"], "silicon-browser");
    ...
    fn test_default_timeout_ms_from_env() {
        // When SILICON_BROWSER_DEFAULT_TIMEOUT is set, DaemonState should use it
        env::set_var("SILICON_BROWSER_DEFAULT_TIMEOUT", "3000");
        let state = DaemonState::new();
        assert_eq!(state.default_timeout_ms, 3000);
        env::remove_var("SILICON_BROWSER_DEFAULT_TIMEOUT");
    }

    #[test]
    fn test_default_timeout_ms_fallback() {
        // When SILICON_BROWSER_DEFAULT_TIMEOUT is unset, DaemonState uses 30000
        env::remove_var("SILICON_BROWSER_DEFAULT_TIMEOUT");
        let state = DaemonState::new();
    ...
    async fn test_auto_dialog_enabled_by_default() {
        let guard = EnvGuard::new(&["SILICON_BROWSER_NO_AUTO_DIALOG"]);
        std::env::remove_var("SILICON_BROWSER_NO_AUTO_DIALOG");
        let state = DaemonState::new();
    ...
    async fn test_auto_dialog_disabled_by_env() {
        let guard = EnvGuard::new(&["SILICON_BROWSER_NO_AUTO_DIALOG"]);
        guard.set("SILICON_BROWSER_NO_AUTO_DIALOG", "1");
        let state = DaemonState::new();
        assert!(
            !state.auto_dialog,
            "auto_dialog should be false when SILICON_BROWSER_NO_AUTO_DIALOG=1"
        );
        drop(guard);
    }
    ...
    async fn test_auto_dialog_disabled_by_env_true() {
        let guard = EnvGuard::new(&["SILICON_BROWSER_NO_AUTO_DIALOG"]);
        guard.set("SILICON_BROWSER_NO_AUTO_DIALOG", "true");
        let state = DaemonState::new();
        assert!(
            !state.auto_dialog,
            "auto_dialog should be false when SILICON_BROWSER_NO_AUTO_DIALOG=true"
        );
        drop(guard);
    }
    ...
    async fn test_auto_dialog_not_disabled_by_random_value() {
        let guard = EnvGuard::new(&["SILICON_BROWSER_NO_AUTO_DIALOG"]);
        guard.set("SILICON_BROWSER_NO_AUTO_DIALOG", "no");
        let state = DaemonState::new();
        assert!(
            state.auto_dialog,
