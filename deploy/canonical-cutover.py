#!/usr/bin/env python3
"""Rehearse or apply the canonical IAM upgrade without starting APIs or workers.
...
MUTABLE = {
    'identity_projection': {'principal_id', 'updated_at'},
...
def retained_rows(database):
    with closing(sqlite3.connect(database.resolve().as_uri() + '?mode=ro', uri=True)) as db:
        ...
        for (table,) in db.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"):
            if table.startswith('sqlite_') or table == '_sqlx_migrations':
                continue
...
def convert(binary, database, expected, runtime):
    env = runtime.copy()
    env['SB_DATABASE_URL'] = 'sqlite://' + str(database.resolve()) + '?mode=rw'
    subprocess.run([str(binary), '--canonicalize-identities-only'], env=env, check=True)
...
def main():
    ...
    with open('/var/lock/silicon-browser-deploy.lock', 'w') as lock:
        ...
        for snapshot, suffix in files:
            ...
            convert(binary, rehearsal, expected, runtime)
        ...
        if args.action == 'apply':
            try:
                ...
                for _, live, expected in pairs:
                    convert(binary, live, expected, runtime)
