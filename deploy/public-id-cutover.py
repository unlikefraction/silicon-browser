...
def main():
    ...
    with open('/var/lock/silicon-browser-deploy.lock', 'w') as lock:
        ...
        # The authorized September reset leaves one production store. Refuse
        # applying a production mapping if any test store has since appeared.
        if len(files) != 1 or files[0][1] != '.db':
            raise SystemExit('Expected only the production store after test reset')
