...
class RetentionTests(unittest.TestCase):
    ...
    def test_new_world_marker_is_migration_metadata(self):
        before = operator.retained_rows(self.database)
        with closing(sqlite3.connect(self.database)) as db, db:
            db.executescript("""
              CREATE TABLE public_identifier_schema(singleton INTEGER, scope_key TEXT, mapping_json TEXT);
              INSERT INTO public_identifier_schema VALUES(1, '', '[]');
            """)
        self.assertEqual(before, operator.retained_rows(self.database))
    ...
    def test_offline_invocation_pins_selected_database_and_mapping(self):
        ...
        self.assertEqual(kwargs['env']['SB_DATABASE_URL'], 'sqlite://' + str(self.database.resolve()) + '?mode=rw')
