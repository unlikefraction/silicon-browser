...
- **`skills` command** - Added `silicon-browser skills` command for discovering and installing agent skills, with built-in evaluation support for testing skills against live browser sessions (#1225, #1227)
...
- **Embedded dashboard** - The observability dashboard is now bundled directly into the CLI binary using `rust-embed`, eliminating the need for `dashboard install`. The dashboard is available immediately after installing silicon-browser (#1169)
...
- **AI chat command** - Added `chat` command for AI-powered browser automation. Supports single-shot mode (`chat "open google.com"`) and an interactive REPL. The AI agent can execute any silicon-browser command via tool calls. Requires `AI_GATEWAY_API_KEY`. Configure the model with `--model` or `AI_GATEWAY_MODEL` (#1160, #1163)
...
- Fixed **`SILICON_BROWSER_DEFAULT_TIMEOUT`** not being applied to `wait` commands. The environment variable now propagates to all wait variants (`wait`, `wait --url`, `wait --text`, `wait --load`, `wait --fn`, `wait --download`) (#1153)
...
- Fixed **`auto_launch()`** not honouring `SILICON_BROWSER_PROVIDER` for cloud providers, causing non-launch commands to fall back to local Chrome instead of connecting via the provider API (#1126)
...
- **Auto-dismissal for alert and beforeunload dialogs** - JavaScript `alert()` and `beforeunload` dialogs are now automatically accepted to prevent the agent from blocking indefinitely. `confirm` and `prompt` dialogs still require explicit `dialog accept/dismiss` commands. Disable with `--no-auto-dialog` flag or `SILICON_BROWSER_NO_AUTO_DIALOG` environment variable (#1075)
- **Puppeteer browser cache fallback** - Chrome discovery now searches `~/.cache/puppeteer/chrome/` (or `PUPPETEER_CACHE_DIR`) for Chrome binaries, so users with an existing Puppeteer installation can use silicon-browser without a separate install step (#1088)
...
```bash
silicon-browser dashboard install
silicon-browser dashboard start
silicon-browser open example.com    # session appears in dashboard
silicon-browser dashboard stop
```

- **Runtime stream management** - Added `stream enable`, `stream disable`, and `stream status` commands to control WebSocket streaming at runtime. Streaming is now always enabled by default; `SILICON_BROWSER_STREAM_PORT` overrides the port instead of toggling the feature (#951)
...
```bash
silicon-browser network requests --type xhr,fetch --method POST
silicon-browser network request <requestId>
```
...
- **Built-in `upgrade` command** - Added `silicon-browser upgrade` to self-update the CLI; automatically detects your installation method (npm, Homebrew, or Cargo) and runs the appropriate update command (#898)
...
```bash
echo '[["open","example.com"],["snapshot"]]' | silicon-browser batch --json
```
...
silicon-browser network har stop ./trace.har
```

### New Features

- **iframe support** -- CLI interactions and snapshots now traverse into iframe content, enabling automation of cross-frame pages.
- **`--idle-timeout` flag** -- Automatically shut down the daemon after a period of inactivity. Accepts human-friendly formats such as `10s`, `3m`, `1h`, or raw milliseconds. Also available as `SILICON_BROWSER_IDLE_TIMEOUT_MS`.
...
silicon-browser is now 100% native Rust. The Node.js/Playwright daemon has been completely removed -- no Node.js runtime or Playwright dependency is required to run the daemon. The Rust native daemon is now the only implementation.
...
```bash
npm install -g silicon-browser      # 7 MB install
silicon-browser install              # download Chrome
...
- **Browserless.io provider** -- Added browserless.io as a browser provider, supported in both Node.js and native daemon paths. Connect to remote Browserless instances using the `--provider browserless` flag or `SILICON_BROWSER_PROVIDER=browserless` environment variable.

```bash
export BROWSERLESS_API_KEY="your-api-key"
silicon-browser --provider browserless open example.com
silicon-browser --provider browserless screenshot ./page.png
```
...
- **Screenshot output configuration** -- New global flags for persistent screenshot settings: `--screenshot-dir`, `--screenshot-quality`, and `--screenshot-format`. Also available as environment variables `SILICON_BROWSER_SCREENSHOT_DIR`, `SILICON_BROWSER_SCREENSHOT_QUALITY`, and `SILICON_BROWSER_SCREENSHOT_FORMAT`.
...
- **Lightpanda browser engine support** -- Added `--engine <name>` flag to select the browser engine (`chrome` by default, or `lightpanda`), implying `--native` mode. Configurable via `SILICON_BROWSER_ENGINE` environment variable.
...
- **Native Rust daemon (experimental).** A pure Rust daemon that communicates with Chrome directly via the Chrome DevTools Protocol (CDP), eliminating Node.js and Playwright dependencies entirely. Enable with `--native`, `SILICON_BROWSER_NATIVE=1`, or `"native": true` in your config file. Supports 150+ commands with full parity to the default Node.js daemon.

```bash
# Via flag
silicon-browser --native open example.com

# Via environment variable
export SILICON_BROWSER_NATIVE=1
silicon-browser open example.com
```

Or add to `silicon-browser.json`:
...
- The native and Node.js daemons share the same session socket. Use `silicon-browser close` before switching between modes.
...
- **`--download-path` option** -- Set a default download directory via flag, `SILICON_BROWSER_DOWNLOAD_PATH` env var, or `downloadPath` config key. Without it, downloads go to a temporary directory deleted when the browser closes.
- **`--selector` flag for scroll** -- Scroll within a specific container element instead of the page: `silicon-browser scroll down 500 --selector "div.scroll-container"`
...
# Security flags
silicon-browser --content-boundaries --allowed-domains "example.com,*.example.com" --max-output 50000 open https://example.com

# Download path
silicon-browser --download-path ./downloads open https://example.com

# Scroll within container
silicon-browser scroll down 500 --selector "div.content"
```

### Environment Variables

Six new environment variables for security configuration: `SILICON_BROWSER_CONTENT_BOUNDARIES`, `SILICON_BROWSER_MAX_OUTPUT`, `SILICON_BROWSER_ALLOWED_DOMAINS`, `SILICON_BROWSER_ACTION_POLICY`, `SILICON_BROWSER_CONFIRM_ACTIONS`, `SILICON_BROWSER_CONFIRM_INTERACTIVE`.
...
- **`--color-scheme` flag** -- Persistent dark/light mode preference across browser sessions via flag or `SILICON_BROWSER_COLOR_SCHEME` env var.

```bash
silicon-browser keyboard type "Hello world"
silicon-browser keyboard inserttext "pasted text"
silicon-browser --color-scheme dark open https://example.com
```

### Bug Fixes

- Fixed IPC EAGAIN errors (os error 35/11) with backpressure-aware socket writes, command serialization, and lowered default Playwright timeout to 25s (configurable via `SILICON_BROWSER_DEFAULT_TIMEOUT`).
...
- **Annotated screenshots** -- `--annotate` flag overlays numbered labels on interactive elements and prints a legend mapping each label to its element ref. Enables multimodal AI models to reason about visual layout while using the same `@eN` refs for subsequent interactions. Also settable via `SILICON_BROWSER_ANNOTATE` env var.
...
- **Configuration file support** -- Automatic loading from user (`~/.silicon-browser/config.json`) and project (`./silicon-browser.json`) directories with priority-based merging.
...
silicon-browser state show myapp
...
```bash
# Via -p flag
silicon-browser -p kernel open https://example.com

# Via environment variable
export SILICON_BROWSER_PROVIDER=kernel
...
```bash
silicon-browser --ignore-https-errors open https://localhost:3000
```

- **Enhanced cookie management** - Extended `cookies set` command with additional flags for setting cookies before page load

```bash
silicon-browser cookies set session_id "abc123" --url https://app.example.com --httpOnly --secure
silicon-browser cookies set token "xyz" --domain .example.com --path /api --expires 1735689600
```
...
# Via environment variable
export SILICON_BROWSER_PROVIDER=browserbase
silicon-browser open https://example.com
```
...
```bash
silicon-browser --cdp "wss://browser-service.com/cdp?token=..." snapshot
```

- **`download` command** - Trigger downloads and wait for completion

```bash
silicon-browser download @e1 ./file.pdf
silicon-browser wait --download ./output.zip --timeout 30000
```

- **Browser launch configuration** - Fine-grained control over browser startup

```bash
silicon-browser --args "--disable-gpu,--no-sandbox" open example.com
silicon-browser --user-agent "Custom UA" open example.com
silicon-browser --proxy-bypass "localhost,*.internal" open example.com
```
...
silicon-browser snapshot  # No --cdp needed after connect
```

- **`--proxy` flag** - Configure browser proxy with optional authentication

```bash
silicon-browser --proxy http://user:pass@proxy.com:8080 open example.com
```
...
- Respect `SILICON_BROWSER_HEADED` environment variable
