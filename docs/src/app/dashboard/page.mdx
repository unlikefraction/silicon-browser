...
Monitor silicon-browser sessions in real time with a local web dashboard showing a live browser viewport and command activity feed.
...
By default each session binds its WebSocket stream server to an OS-assigned port. To use a specific port, set the `SILICON_BROWSER_STREAM_PORT` environment variable:
