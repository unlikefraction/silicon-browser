...
[Browserless](https://browserless.io) provides cloud browser infrastructure with a Sessions API. Use it when running silicon-browser in environments where a local browser isn't available.
...
```bash
export SILICON_BROWSER_PROVIDER=browserless
export BROWSERLESS_API_KEY="your-api-token"
...
When enabled, silicon-browser connects to a Browserless cloud session instead of launching a local browser. All commands work identically.
