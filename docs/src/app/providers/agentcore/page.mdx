...
[AWS Bedrock AgentCore](https://aws.amazon.com/bedrock/agentcore/) provides cloud browser sessions with SigV4 authentication. Use it when running silicon-browser in AWS environments or when you need managed cloud browsers backed by AWS infrastructure.
...
```bash
silicon-browser -p agentcore open https://example.com
```

Or use environment variables for CI/scripts:

```bash
export SILICON_BROWSER_PROVIDER=agentcore
...
```bash
AGENTCORE_PROFILE_ID=my-profile silicon-browser -p agentcore open https://example.com
```
...
If using SSO, run `aws sso login` before launching silicon-browser. Set `AWS_PROFILE` to select a specific named profile.
...
AGENTCORE_PROFILE_ID=my-profile silicon-browser -p agentcore open https://x.com/home

# With explicit region
AGENTCORE_REGION=eu-west-1 silicon-browser -p agentcore open https://example.com

# With SSO profile
AWS_PROFILE=my-sso-profile silicon-browser -p agentcore open https://example.com
```

When enabled, silicon-browser connects to an AgentCore cloud browser session instead of launching a local browser. All commands work identically.
