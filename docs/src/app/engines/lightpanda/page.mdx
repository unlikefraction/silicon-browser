...
silicon-browser manages Lightpanda the same way it manages Chrome -- spawning the process, connecting via CDP, and shutting it down. All downstream commands (snapshot, click, fill, screenshot, etc.) work through the same CDP protocol path.

## Installation

Install the Lightpanda binary before using it with silicon-browser:
...
silicon-browser --engine lightpanda snapshot
silicon-browser --engine lightpanda screenshot
```
...
Or in your `silicon-browser.json` config:
...
silicon-browser returns a clear error if you combine `--engine lightpanda` with unsupported flags.
