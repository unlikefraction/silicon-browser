...
Chrome (and Chromium) is the default browser engine. silicon-browser discovers, launches, and manages the Chrome process automatically via the Chrome DevTools Protocol (CDP).

## Binary Discovery

When no `--executable-path` is provided, silicon-browser searches for Chrome in this order:
...
If Chrome is not found, run `silicon-browser install` to download Chrome from Chrome for Testing.
