...
To bind to a specific port, set `SILICON_BROWSER_STREAM_PORT`:

```bash
SILICON_BROWSER_STREAM_PORT=9223 silicon-browser open example.com
```

You can also manage streaming at runtime:

```bash
silicon-browser stream status            # Show streaming state and bound port
silicon-browser stream enable --port 9223  # Re-enable on a specific port
silicon-browser stream disable           # Stop streaming for the session
```
...
`silicon-browser stream status --json` returns data like:
...
```typescript
import { BrowserManager } from 'silicon-browser';
