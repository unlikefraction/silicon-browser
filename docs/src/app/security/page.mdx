...
silicon-browser includes security features to protect against credential exposure, prompt injection via untrusted page content, and unauthorized browser actions.

All security features are opt-in. By default, silicon-browser imposes no restrictions on navigation, actions, or output. Enable these features as needed for your deployment -- existing workflows are unaffected until you explicitly activate a feature.
...
- **Domain filter timing on remote connections.** When connecting to a pre-existing browser via CDP or a cloud provider, pages may have already loaded content before the domain filter is installed. silicon-browser navigates disallowed pages to `about:blank` after the filter is active, but resources loaded before that point are not retroactively blocked.
...
```bash
# Save credentials (encrypted if SILICON_BROWSER_ENCRYPTION_KEY is set)
...
```bash
silicon-browser auth save myapp \
...
Profiles are stored in `~/.silicon-browser/auth/` and always encrypted with AES-256-GCM. If `SILICON_BROWSER_ENCRYPTION_KEY` is not set, a key is auto-generated at `~/.silicon-browser/.encryption-key` on first use. Back up this file or set the environment variable explicitly for portability.
...
```
--- SILICON_BROWSER_PAGE_CONTENT nonce=a1b2c3d4 origin=https://example.com ---
[snapshot / text / html / eval output here]
--- END_SILICON_BROWSER_PAGE_CONTENT nonce=a1b2c3d4 ---
```
...
```bash
silicon-browser --content-boundaries snapshot
# or
...
```bash
silicon-browser --allowed-domains "example.com,*.example.com,github.com" open https://example.com
# or
...
```bash
silicon-browser --action-policy ./policy.json open https://example.com
# or
...
```bash
silicon-browser --confirm-actions eval,download --confirm-interactive eval "document.title"
# Prompts: Allow? [y/N]
```
...
```bash
silicon-browser --max-output 50000 get text body
# or
...
<table>
  ...
  <tbody>
    <tr><td><code>SILICON_BROWSER_CONTENT_BOUNDARIES</code></td><td>Wrap page output in boundary markers</td></tr>
    <tr><td><code>SILICON_BROWSER_MAX_OUTPUT</code></td><td>Max characters for page output</td></tr>
    <tr><td><code>SILICON_BROWSER_ALLOWED_DOMAINS</code></td><td>Comma-separated allowed domain patterns</td></tr>
    <tr><td><code>SILICON_BROWSER_ACTION_POLICY</code></td><td>Path to action policy JSON file</td></tr>
    <tr><td><code>SILICON_BROWSER_CONFIRM_ACTIONS</code></td><td>Comma-separated action categories requiring confirmation</td></tr>
    <tr><td><code>SILICON_BROWSER_CONFIRM_INTERACTIVE</code></td><td>Enable interactive confirmation prompts</td></tr>
    <tr><td><code>SILICON_BROWSER_ENCRYPTION_KEY</code></td><td>64-char hex key for AES-256-GCM encryption (auth vault + sessions)</td></tr>
  </tbody>
</table>
