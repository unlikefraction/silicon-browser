...
Run silicon-browser from a Next.js app on Vercel using Vercel Sandbox.
A Linux microVM spins up on demand, runs silicon-browser + Chrome, and
...
step before silicon-browser can launch Chrome. Use a sandbox snapshot
...
export async function snapshotUrl(url: string) {
  return withBrowser(async (sandbox) => {
    ...
    const result = await sandbox.runCommand("silicon-browser", [
      "snapshot", "-i", "-c",
    ]);
...
silicon-browser + Chromium from scratch (~30 seconds). A **sandbox snapshot**
is a saved VM image with everything pre-installed -- like a Docker image
for Vercel Sandbox. When `SILICON_BROWSER_SNAPSHOT_ID` is set, the sandbox
boots from that image instead of installing, bringing startup down to
sub-second.

This is different from an silicon-browser *accessibility snapshot* (which
...
silicon-browser + Chromium, saves the VM state, and prints the snapshot ID:
...
export async function GET() {
  const result = await withBrowser(async (sandbox) => {
    await sandbox.runCommand("silicon-browser", [
      "open", "https://example.com/pricing",
    ]);
    const snap = await sandbox.runCommand("silicon-browser", [
      "snapshot", "-i", "-c",
    ]);
    await sandbox.runCommand("silicon-browser", ["close"]);
...
<table>
  ...
  <tbody>
    <tr><td><code>SILICON_BROWSER_SNAPSHOT_ID</code></td><td>Sandbox snapshot ID for sub-second startup (see above)</td></tr>
...
[`examples/environments/`](https://github.com/unlikefraction/silicon-browser/tree/main/examples/environments).
